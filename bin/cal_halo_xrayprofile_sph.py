...
for mf in [13.0, 13.5]:
    ...
    props_names = ['cts']#['part_masses','part_dens','part_temperatures', 'cts']
    ...
    for q, xbins in enumerate([xbins_mean, xbins_med]):
        for prop in props_names:
            def cal_xraylum_excl(k):
                ...
                plt.hist(np.linalg.norm(np.array([olddf_part['part_xcoords'], olddf_part['part_ycoords'], olddf_part['part_zcoords']]).T-halo_cen, axis=1), bins = bins, alpha = 0.5)
                plt.xscale('log')
                plt.yscale('log')
                plt.savefig('1.png')
