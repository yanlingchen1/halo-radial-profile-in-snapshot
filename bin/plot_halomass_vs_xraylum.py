...
for j, line in enumerate(['sumlum_o8', 'sumlum_o7f', 'sumlum_fe17']): 
    ...
    for i, mf in enumerate(mass_filter):
        print(mf)
        workpath = f'/cosma8/data/dp004/dc-chen3/work/bin/halo-radial-profile-in-snapshot/results/results_add_xraylum_sb_230509/xraysb_csvs_{mf}_groups_1028halos/results/results_add_xraylum_sb_230509/xraysb_csvs_{mf}groups_1028halos' 
        df = pd.read_csv(f'{workpath}/xray_linelum_inr200c_snapshot75_halomass_btw_{int(mf*10)}_{int((mf+0.5)*10)}_230515.csv')   
        lum = df[line] 
        my_med[i] = np.log10(np.median(lum)/np.array(linesbins[line]).mean()/4/np.pi/ (1.48e27)**2*1e4/1.602e-9*1e5)
        my_hierr[i] = np.log10(np.percentile(lum, 90)/np.array(linesbins[line]).mean()/4/np.pi/ (1.48e27)**2*1e4/1.602e-9*1e5) - np.log10(np.median(lum)/np.array(linesbins[line]).mean()/4/np.pi/ (1.48e27)**2*1e4/1.602e-9*1e5)
        my_loerr[i] = np.log10(np.median(lum)/np.array(linesbins[line]).mean()/4/np.pi/ (1.48e27)**2*1e4/1.602e-9*1e5) - np.log10(np.percentile(lum, 10)/np.array(linesbins[line]).mean()/4/np.pi/ (1.48e27)**2*1e4/1.602e-9*1e5)
