...
mul_props_names = ['part_temperatures', 'nH_dens']
...
# mask_names = ['incl']
mask_names = ['rhp'] # 'total', , 'rhp_xraylum'
...
for mf in [13.5]:
    ...
    savepath = f'{workpath}/profiles_230809_{mf}_paratest_r500c_from1028draw32'
                ...
                for n, mask_name in enumerate(mask_names):
                    ...
                    def cal_xraylum_mul(k):
                        ...
                        for i in range(len(bins)-1):
                            ...
                            radmsk = msk_in_sph_new(np.array([olddf_part['part_ycoords'], olddf_part['part_zcoords'], olddf_part['tot_abun_to_solar']]).T, halo_cen, bins[i], bins[i+1])
                            # load mask
                    ...
                    # formal run
                    output = np.zeros((len(xbins) - 1, 32)) #len(haloids)

                    with concurrent.futures.ProcessPoolExecutor(32) as executor:
                        for k, result in enumerate(executor.map(cal_xraylum_mul, np.arange(32))): # len(haloids)
                            output[:,k] = result
