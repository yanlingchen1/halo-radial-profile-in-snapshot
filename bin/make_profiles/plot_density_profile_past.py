...
# define parameters
massfilter = [13.0, 13.5, 14.0,14.5]
props = ['part_masses']
    ...
    for prop in props:
        ...
        if prop == 'part_masses':
            for shape in ['sph', 'cyl']:
                ...
                for k, type in enumerate(['excl', 'incl']):
                    ...
                    for i, binning in enumerate(xbins.keys()):
                        ...
                        if binning == '010dex':
                            ...
                            xbins_ex = np.tile(xs_ins, (len(r200c),1)) * 1e3
                            shell_vol = cal_vol(shape, xbins_ex)
                            plt.plot(np.log10(xs/np.median(r200c)), np.log10(np.nanmedian(prof/shell_vol.T, axis=1)), c = cb[i], label = f'{shape}_{binning}_{xbins[binning][1]}_{type}', linestyle = lstyles[k], linewidth = 2, alpha = 0.8)
                        elif binning == '025dex':
                            xs = xbins[binning][0]
                            xs_ins = np.insert(xs,0,0)
                            xbins_ex = np.tile(xs_ins, (len(r200c),1)) * 1e3
                ...
                plt.ylabel('$ \\rm log_{10}(M_{gas} / V_{shell})\ [M_{\odot}/kpc^3]$ ')

                plt.title(f'$\\rm 1e{mf}-{mf+0.5} M_{{\odot}}$ gas mass density v.s. r')
                plt.legend()
                plt.savefig(f"{savepath}/{prop}_1e{mf}_{shape}_gasmass_dens_vs_r_past.png")
