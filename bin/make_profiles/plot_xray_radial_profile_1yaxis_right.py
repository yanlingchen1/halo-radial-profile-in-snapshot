...
for i, line in enumerate(['fe17', 'o7f', 'o8']):
    for part in ['incl', 'excl']:
        ...
        prop_med = unit2wij_ri(np.nanmedian(dat, axis=1), line, bins)
        prop_hi = unit2wij_ri(np.percentile(dat, 84, axis=1), line, bins)
        prop_lo = unit2wij_ri(np.percentile(dat, 16, axis=1), line, bins)
        prop_mean = unit2wij_ri(np.nanmean(dat, axis=1), line, bins)
        ...
        y_nat = np.array(nat_mean[line])[:,1]
        ...
        ax1.legend()
        ax1.set_xscale('log')
        ax1.set_ylabel(f'$\\rm log_{{10}}$SB [photons/$\\rm m^2/100ks/10\ arcmin^2$]')
        ax1.set_xlabel(f'$\\rm pkpc$')
    plt.suptitle(f'halomass $10^{{{mf:.1f}-{(mf+0.5):.1f}}}$ solarmass {line}')
    plt.savefig(f'{workpath}/png/halomass{int(mf*10)}-{int((mf+0.5)*10)}_{line}_xraylum_medians_1yaxis_right.png')
    plt.close()