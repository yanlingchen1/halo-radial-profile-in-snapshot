...
def compute_lum(interp_rest_range, data, table_type, z, where):
    ...
    if lum is False:
        ...
        return lum[0], exclude_rhp_msk, abun_to_solar, tot_abun_to_solar
...
def cal_halo_summass(sid):
    ...
    msk = msk_in_sph_new(np.array(data.gas.coordinates), (gasmass_center[:,0][sid],gasmass_center[:,1][sid],gasmass_center[:,2][sid]), 0, r200c_sp[sid]*11)
    ...
    for line in linesbins.keys():
        lumdict[line], exclude_rhp_msk, abun_to_solar, tot_abun_to_solar = compute_lum(linesbins[line], data, 'lines', reds, msk)

    return data.gas.particle_ids[msk], lumdict['o7f'], lumdict['o8'], lumdict['fe17'], exclude_rhp_msk, data.gas.masses[msk], data.gas.densities[msk], nH_densities, data.gas.temperatures[msk], abun_to_solar[:,0], abun_to_solar[:,1], abun_to_solar[:,2], abun_to_solar[:,3], abun_to_solar[:,4], abun_to_solar[:,5], abun_to_solar[:,6], abun_to_solar[:,7], abun_to_solar[:,10],  data.gas.coordinates[:,0][msk], data.gas.coordinates[:,1][msk], data.gas.coordinates[:,2][msk], tot_abun_to_solar
...
for mf in mass_filter:
    ...
    savepath = f'{workpath}/results/redshift_01/{sim}/xraysb_csvs_230809_{mf}_groups_{halonum}halos_sph'
    ...
    output['r500c'] = r500c_sp[np.array(halo_sel_ids-1, dtype = int)]
    output['m500c'] = m500c_sp[np.array(halo_sel_ids-1, dtype = int)]
    output['r200c'] = r200c_sp[np.array(halo_sel_ids-1, dtype = int)]
    output['m200c'] = m200c_sp[np.array(halo_sel_ids-1, dtype = int)]
    ...
    # print(halodoc['o7f'], halodoc['o8'], halodoc['fe17'])
