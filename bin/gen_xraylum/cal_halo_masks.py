...
def cal_halo_summass(sid):
    ...
    exclude_rhp_msk =  ~((abs(t_agn-t_par).value*1000<15) & (~np.isnan(a_agn)) & ((np.power(10, 6.94455)<data.gas.temperatures[msk].value) | (np.power(10, 8.24455)>data.gas.temperatures[msk].value)))
    
    return data.gas.particle_ids[msk], calxray_msk, exclude_rhp_msk
...
for mf in mass_filter:
    ...
    print(np.sum(msk))
    if np.sum(msk)>halonum:
        halo_rands = np.random.randint(np.sum(msk), size=halonum)
        halo_sel_ids = [soap_ids[msk][halo_rands]]
    else:
        halo_sel_ids = [soap_ids[msk]]
    ...
    with concurrent.futures.ProcessPoolExecutor(1) as executor:
        for i, result in enumerate(executor.map(cal_halo_summass, (np.array(halo_sel_ids)-1).astype(int))):
            halodoc = {}
            halodoc['particle_ids'], halodoc['calxray_msk'], halodoc['exclude_rhp_msk']= result
            df1 = pd.DataFrame.from_dict(halodoc)
