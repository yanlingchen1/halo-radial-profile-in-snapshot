...
for i, line in enumerate(['fe17', 'o7f', 'o8']):
    for part in ['incl', 'excl']:
        ...
        prop_med = unit2wij_le(np.nanmedian(dat, axis=1), line, bins)
        prop_hi = unit2wij_le(np.percentile(dat, 84, axis=1), line, bins)
        prop_lo = unit2wij_le(np.percentile(dat, 16, axis=1), line, bins)
        prop_mean = unit2wij_le(np.nanmean(dat, axis=1), line, bins)
        ...
        ax1.legend()
        ax1.set_xscale('log')
        ax1.set_ylabel(f'$\\rm log_{{10}}$SB [photons//$\\rm cm^2/sr$]')
        ax1.set_xlabel(f'$\\rm pkpc')
        plt.suptitle(f'halomass $10^{{{mf:.1f}-{(mf+0.5):.1f}}}$ solarmass {mode}, {time.asctime()}')
        plt.savefig(f'{workpath}/png/halomass{int(mf*10)}-{int((mf+0.5)*10)}_{mode}_xraylum_medians_1yaxis_leftori.png')
        plt.close()